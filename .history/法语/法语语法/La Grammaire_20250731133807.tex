...
\textit{pendant que}, \textit{sans que}, \textit{nonobstant que}(arch.), \textit{en dépit que}(arch.), \textit{malgré que} (部分作者如Prout, Gide使用)
\begin{enumerate}
    \item 只有在对立中才能解释为concession 
    \begin{itemize}
        ...
        \item les élèves de la classe-expérimentale se sont comportés autrement, ont produit autre- ment, que ceux des classes-témoins, même en fin d’année, [en dépit que ces élèves de la classe- expérimentale, aient été, au préalable et durant neuf mois, connectés avec des œuvres d’art renouvelées toutes les semaines].
    \end{itemize}
\end{enumerate}
