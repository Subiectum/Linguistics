...
\begin{tabular}{|p{7.5cm}|p{7.5cm}|} % Adjusted column widths for better fit
\hline
\textbf{VERBE SANS COMPLÉMENT PROPRE} & \textbf{VERBE AVEC COMPLÉMENT PROPRE} \\
\hline
\multicolumn{2}{|c|}{\textbf{SUJET PRÉDICATIF}} \\ % Main category as a merged cell
\hline
\textit{(un événement) arriver, (un accident) avoir lieu, (un bruit) courir,} & \textit{(une tempête) frapper (quelque part), (un événement) prendre place} \\
\textit{(le match) se dérouler, (un spectacle) se donner, (un orage) éclater,} & \textit{(quelque part), (une fête) tomber (tel jour), (un ouragan) toucher} \\
\textit{(une histoire) se passer, (un événement) se produire, (le silence) régner,} & \textit{(un endroit)} \\
\textit{(un événement) survenir, (une réunion) se tenir} & \\
\hline
\multicolumn{2}{|c|}{\textbf{COMPLÉMENT DIRECT PRÉDICATIF}} \\ % Main category as a merged cell
\hline
\textit{prendre (la fuite), produire (un effet), promulguer (un décret), prononcer} & \textit{accorder (son pardon) (à), administrer (une correction) (à),} \\
\textit{(un discours), propager (une rumeur), ratifier (un accord), relever (un} & \textit{adresser (des reproches) (à), allonger (une gifle) (à),} \\
\textit{défi), rendre (un avis), ressentir (de la tristesse), soulever (une objection),} & \textit{allouer (une indemnité) (à), asséner (une gifle) (à), balancer} \\
\textit{souscrire (une assurance), soutenir (une idée), subir (un orage), tisser} & \textit{(une plaisanterie) (à), concéder (un prêt) (à), conférer} \\
\textit{(des intrigues), tramer (un complot)} & \textit{(une distinction) (à), cracher (des injures) (à), dicter (ses ordres) (à),} \\
& \textit{dispenser (des consolations) (à), donner (son accord) (à), ficher} \\
& \textit{(des coups) (à), filer (des coups) (à), flanquer (des coups) (à), formuler} \\
& \textit{(des remarques) (à), impartir (un délai) (à), infliger (des reproches) (à),} \\
& \textit{intenter (un procès) (à), intimer (un ordre) (à), lâcher (des injures) (à),} \\
& \textit{lancer (un défi) (à), libeller (un message) (à), marteler (un ordre) (à),} \\
& \textit{octroyer (une aide) (à), offrir (son aide) (à), poser (une question) (à),} \\
& \textit{prêter (un appui) (à), procurer (une aide) (à), prodiguer (des conseils) (à),} \\
& \textit{recevoir (un conseil) (de), vouer (de l'admiration) (à)} \\
\hline
\multicolumn{2}{|c|}{\textbf{COMPLÉMENT OBLIQUE PRÉDICATIF}} \\ % Main category as a merged cell
\hline
\textit{s'adonner (à l'escrime), faire étalage (de courage),} & \textit{accabler (qqn) (de reproches), gratifier (qqn) (d'une récompense),} \\
\textit{faire montre (de courage), faire preuve (d'intelligence),} & \textit{nantir (qqn) (de provisions), soumettre (qqn) (à la torture)} \\
\textit{jouir (d'une bonne santé), se livrer (à des confidences),} & \\
\textit{procéder (à une opération), procéder (d'une grande méchanceté)} & \\
\hline
\end{tabular}
