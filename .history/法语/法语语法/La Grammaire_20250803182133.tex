...
\begin{table}[H]
    ...
    \newline [D'ici qu'il pleuve], on a le temps de sortir. \\
    \hline
    ...
    \newline [Les enfants une fois arrivés], on pouvait se détendre. \\
    \hline
...
\subsection{L’ordre des mots au sein des autres syntagmes}

\begin{table}[H]
    \centering
    \small
    \begin{tabular}{|>{\RaggedRight}p{3.5cm}|>{\RaggedRight}p{3.5cm}|>{\RaggedRight}p{3.5cm}|>{\RaggedRight}p{5cm}|}
    \hline
    \rowcolor{cyan!20}
    \textbf{CATÉGORIE} & \textbf{FONCTION} & \textbf{POSITION} & \textbf{EXEMPLES} \\
    \hline
    \rule{0pt}{1.5ex}adjectif, adverbe, determinant, nom & spécifieur du nom & début de syntagme & [le] médecin, [mon] ami, [pareille] aventure, [beaucoup] de bruit, [nombre] de difficultés \\
    \hline
    \rule{0pt}{1.5ex}adjectif, adverbe & ajout au nom ou au SN & mobile & un [délicieux] gâteau, un gâteau [délicieux] \\
    & & & [même] Paul \\
    \hline
    \rule{0pt}{1.5ex}adverbe, SN, SP & ajout à l'adjectif & mobile & [très] grand, [trois fois] supérieur, long [de 2 mètres] \\
    \hline
    \rule{0pt}{1.5ex}SA, nom, SN, subordonnée relative & ajout au nom ou au SN & après le nom & une table [longue de 2 mètres], une fiche [incident], mon ami [le médecin], la table [que j'ai] \\
    \hline
    \rule{0pt}{1.5ex}conjonction de coordination ou préposition & marqueur & début de syntagme & Jean [et] Paul, beaucoup [de] bruit, une place [de] libre \\
    \hline
    \rule{0pt}{1.5ex}SP, SV ou subordonnée & complément du nom & après le nom & le fils [de Marie], le désir [de partir], l'idée [que tu partes] \\
    \hline
    \rule{0pt}{1.5ex}SP, SV ou subordonnée & complément de l'adjectif & après l'adjectif & fier [de lui], fier [de réussir], content [que tu viennes] \\
    \hline
    \rule{0pt}{1.5ex}adverbe, SN, SV, SP ou subordonnée & complément de la préposition & généralement après la préposition & pour [un soir], pour [danser], pour [que l'on danse], pour [longtemps], [trois heures] durant \\
    \hline
    \rule{0pt}{1.5ex}SP ou subordonnée & complément de l'adverbe & après l'adverbe & contrairement [à vous], peut-être [que je viendrai] \\
    \hline
    \end{tabular}
    \caption{L’ordre des principaux éléments dans les syntagmes non verbaux}
\end{table}

\section{L’inversion du sujet}
\begin{enumerate}
    \item l’interrogative partielle ou l’incise 二者都可使用
    \begin{itemize}
        \item Où va-t-elle ? |Où va Marie ?
    \end{itemize}
    \item les subordonnées interrogatives, comparatives ou relatives只能用l’inversion nominale
    \begin{itemize}
        \item * Je sais où va-t-elle. | Je sais où va Marie.
        \item * Voici ce que disait-elle. |Voici ce que disait Marie.
    \end{itemize}
    \item l’interrogative totale只能用 le suffixe pronominal
    \begin{itemize}
        \item Vient-il demain ? |* Vient Paul demain ?
    \end{itemize}
\end{enumerate}
\subsection{Les constructions verbales à sujet suffixé}
\subsubsection{Un verbe à sujet suffixé}
\begin{enumerate}
    \item Sujet suffixé est compatible avec une proforme préverbale interprétée comme complément ou réfléchie
    \begin{itemize}
        \item Nous cachez-vous quelque chose ?
        \item S’est-elle évanouie ?
    \end{itemize}
    \item À la 3e personne, ou au pluriel, elle est compatible avec un sujet nominal avant le verbe, avec lequel elle s’accorde en genre, en nombre et en personne
    \begin{itemize}
        \item Paul vient-il ?
        \item Peut-être Marie viendra-t-elle.
        \item Où ces énergumènes sont-ils encore passés ?
        \item Pierre et moi pouvons-nous entrer ?
    \end{itemize}
    \item  l’impersonnel \textit{il}, ou \textit{ce} et \textit{on}, un sujet préverbal n’est pas possible, sauf emploi personnel de \textit{on} (nous)
    \begin{itemize}
        \item Pleut-il encore ?
        \item Est-ce vrai ?
        \item * Cela est-ce vrai ?
        \item Connait-on le suspect ?
        \item \% Pierre et moi connaitra-t-on le bonheur un jour ?
    \end{itemize}
\end{enumerate}


\paragraph{Les constructions avec un verbe à sujet suffixé}
...
\begin{table}[H]
    ...
    \begin{adjustbox}{max width=\textwidth}
        ...
        \hline
        \rowcolor{cyan!20}
        \textbf{PHRASE} & \textbf{SANS SUJET NOMINAL} & \textbf{AVEC SUJET NOMINAL} \\
        \hline
        déclarative avec adverbe initial & Peut-être viendra-t-elle demain. & Peut-être Marie viendra-t-elle. \\
        \hline
        désidérative sans que & Puisse-t-elle réussir ! & Paul puisse-t-il réussir ! \\
        \hline
        exclamative & Combien a-t-il souffert ! & Combien Luc a-t-il souffert ! \\
        \hline
        incise & Marie, semble-t-il, est venue. & --- \\
        & Marie, dis-je, est venue. & \\
        \hline
        interrogative & Viendra-t-elle ? & Marie viendra-t-elle ? \\
        & Quand prend-il le train ? & Quand Paul prend-il le train ? \\
        \hline
        subordonnée concessive & Aussi riche soit-il, il ne pourra pas payer. & Aussi riche Luc soit-il, il ne pourra pas payer. \\
        \hline
        subordonnée conditionnelle & Viendrait-elle, il ne changerait pas d'avis. & Léa viendrait-elle, il ne changerait pas d'avis. \\
        & Viendrait-elle qu'il ne changerait pas d'avis. & Léa viendrait-elle qu'il ne changerait pas d'avis. \\
        \hline
        subordonnée temporelle & À peine était-il entré, il se mit à chanter. & À peine Luc était-il entré, il se mit à chanter. \\
        & À peine était-il entré qu'il se mit à chanter. & À peine Luc était-il entré qu'il se mit à chanter. \\
        \hline
        \end{tabular}
    \end{adjustbox}
    \caption{Les principales constructions avec verbe à sujet suffixé}
\end{table}


\paragraph{Le verbe à sujet suffixé et l’inversion du sujet nominal}
\begin{enumerate}
    \item aux temps composés, sujet suffixé s’attache à l’auxiliaire, alors que le sujet inversé doit suivre le participe
    \begin{itemize}
        \item Combien a-t-il dépensé ?
        \item Combien a dépensé Paul ?
    \end{itemize}
    \item sujet suffixé est compatible avec un complément nominal, alors que le sujet inversé l’est plus difficilement
    \begin{itemize}
        \item Où a-t-il une maison ?
        \item * Où a Paul une maison ?
    \end{itemize}
    \item sujet suffixé est compatible avec un sujet préverbal
    \begin{itemize}
        \item Paul viendra-t-il ?
    \end{itemize}
\end{enumerate}
...
\subsubsection{Le verbe à sujet suffixé en phrase déclarative ou désidérative}
\paragraph{Le verbe à sujet suffixé  dans les adverbes initiaux}

\begin{enumerate}
    \item une vingtaine d’adverbes: \textit{rarement, apparemment, peut-être, probablement, sans doute, ainsi, difficilement, en vain, vainement, à peine (\textup{obligatoire}), tout juste, tout au plus, aussi, ainsi, encore, déjà, de même, pas davantage}
    
    \begin{itemize}
        \item Rarement la parole humaine fut-elle plus éblouissante et plus rouée 
        \item Sans doute l’animal avait-il compris ce qui allait se passer.
        \item Quand il entra à l’école, à peine savait-il écrire son nom.
        \item En vain tente-t-il de s’échapper. La lumière crue le dévore.
        \item Pendant trois ans, M. Paul Reynaud affirma sa position par plusieurs discours, enfin par des entretiens avec des politiques et des militaires importants. Ainsi prenait-il la figure d’un homme d’État novateur et résolu
        \item Repris huit cents ans plus tard, le travail ne fut achevé que par Darius. Encore le canal était-il destiné à être perpétuellement ensablé
        \item Déjà ai-je eu quelque peine à déguiser à mes invités que les pièces montées de mes chefs étaient pour eux plutôt que pour moi
        \item Urbain Gorenfan a passé, samedi une excellente et courte soirée. De même a-t-il dormi d’un paisible sommeil dans la nuit du samedi au dimanche.
        \item Ce n'est pas l'État qui a écrasé la paysannerie. Pas davantage n'observe-t-on de villes ruinant les campagnes alentour.
        \item * À peine l’enfant savait écrire son nom.
        \item À peine l’enfant savait-il écrire son nom.
        \item On ne le punira pas. Tout au plus le blâmera-t-on.
    \end{itemize}
    \item Le verbe à sujet suffixé est plus difficile avec ceux qui marquent une relation objective entre des situations, relation temporelle (\textit{alors, ensuite}) ou causale (\textit{de ce fait, par conséquent}) 
    \begin{itemize}
        \item \# Le travail ne fut achevé que par Darius. Ensuite le canal fut-il perpétuellement ensablé.
        \item \# Pendant trois ans, M. Paul Reynaud affirma sa position. De ce fait prenait-il la figure d’un homme d’État novateur et résolu.
    \end{itemize}
    \item \textit{ainsi}, \textit{sans doute} et \textit{aussi}也允许l’inversion du sujet nominal
    \begin{itemize}
        \item Devant ces perspectives, on se mit à la recherche de nouvelles sources d’énergie qui ne fussent pas susceptibles d’un rapide épuisement. Ainsi naquirent la houille blanche et la houille verte 
        \item On rentra au chalet et on essaya de faire passer la journée. Sans doute viendrait un moment où on pourrait reprendre la vie normale
    \end{itemize}
    \item Les artistes s’emparent alors des formules nouvelles, les étudient, les adaptent. Aussi naît un style architectural où le sens de l’équilibre et de la mesure contrebalance les origines étrangères.
\end{enumerate}

\paragraph{Le verbe à sujet suffixé dans une phrase désidérative}
\begin{enumerate}
    \item Le verbe au subjonctif exprimant le souhait, le plus souvent \textit{pouvoir} ou \textit{être}
    \begin{itemize}
        \item J’ai trouvé la paix. Puissent-ils la trouver un jour aussi.
        \item Maudit sois-tu./Ainsi soit-il.
        \item Dieu puisse-t-il nous fortifier ! 
    \end{itemize}
    \item Un sujet nominal ne serait possible qu’en inversion ou dans une construction archaïsante
    \begin{itemize}
        \item Puisse le Ciel nous fortifier ! | \# Le Ciel puisse nous fortifier !
        \item Béni soit le temps où le garçonnet lui tombe sur les bras en pleine débâcle
    \end{itemize}
    \item Le sujet nominal y précède l’infinitif, comme dans le cas du verbe à sujet suffixé, alors qu’il le suit dans les autres types d’inversion nominale
    \begin{itemize}
        \item Puisse cet exemple être suivi !
        \item Puisse-t-il être suivi !
        \item Comment pourra être suivi cet exemple ?
        \item * Comment pourra cet exemple être suivi ?
    \end{itemize}
\end{enumerate}

\subsubsection{Le verbe à sujet suffixé en phrase interrogative ou exclamative}

\paragraph{Le verbe à sujet suffixé en phrase interrogative totale ou alternative}
\begin{enumerate}
    \item Il est compatible avec un sujet préverbal.
    \begin{itemize}
        \item Viendra-t-elle ce soir ?
        \item Veux-tu du thé ou du café ?
        \item Marie viendra-t-elle ce soir ?
    \end{itemize}
    \item Il n’apparait pas dans l’interrogative subordonnée
    \begin{itemize}
        \item * Il a demandé [si Marie venait-elle].
    \end{itemize}
    \item Il n'est pas compatible avec la forme \textit{est-ce que}
    \begin{itemize}
        \item Est-ce qu’elle viendra ?
        \item * Est-ce que viendra-t-elle ?
    \end{itemize}
    \item L’inversion du sujet nominal n’est pas possible 
    \begin{itemize}
        \item * Viendra Marie ce soir ?
    \end{itemize}
\end{enumerate}


\paragraph{Le verbe à sujet suffixé en phrase interrogative partielle}
\begin{enumerate}
    \item Il s’emploie avec un mot interrogatif initial, mais il n’est pas possible lorsque le mot interrogatif suit le verbe
    \begin{itemize}
        \item Où as-tu rencontré Marie ?
        \item * As-tu rencontré Marie où ?
    \end{itemize}
    \item Il est incompatible avec les interrogatifs en \textit{est-ce que}
    \begin{itemize}
        \item Qu’est-ce qu’il a mangé ?
        \item * Qu’est-ce qu’a-t-il mangé ?
    \end{itemize}
    \item Son usage avec un sujet préverbal est limité avec un interrogatif sujet
    \begin{enumerate}
        \item impossible avec \textit{qui} sujet
        \begin{itemize}
            \item Qui Paul a-t-il rencontré ?
            \item * Qui vient-il ?
        \end{itemize}
        \item possible avec \textit{lequel}
        \begin{itemize}
            \item Lequel de tes élèves a-t-il échoué ?
        \end{itemize}
        \item plus difficile avec \textit{quel} + nom ou \textit{combien} + de + nom
        \begin{itemize}
            \item \% Quelle solution a-t-elle été mise en place en attendant ? 
            \item \% Combien d’élèves ont-ils échoué ?
        \end{itemize}
    \end{enumerate}
    \item dans l’interrogative non questionnante
    \begin{enumerate}
        \item dans interrogatives exclamants, 必须用sujet suffixé, 不能用 \textit{est-ce que}
        \begin{itemize}
            \item Est-il idiot, ce type !
            \item A-t-il jamais réussi un bon coup !
            \item \# Est-ce qu’il est idiot, ce type !
        \end{itemize}
        \item dans les questions rhétoriques, 必须用sujet suffixé
        \begin{itemize}
            \item Comment veux-tu que je fasse !
        \end{itemize}
        \item dans une interrogative introduite par l’adverbe \textit{que} (pourqoi) avec un \textit{ne} négatif et qui exprime un regret, 必须用sujet suffixé. Un sujet préverbal ne serait pas possible
        \begin{itemize}
            \item Que ne suis-je riche !
            \item Que n’es-tu restée aveugle – à jamais ! à jamais !
            \item * Que je ne suis riche !
            \item * Que Paul n’est-il riche !
        \end{itemize}
    \end{enumerate}
\end{enumerate}

\paragraph{Le verbe à sujet suffixé dans une phrase exclamative}
\begin{enumerate}
    \item possible avec \textit{combien, que, quel}. 
    \begin{itemize}
        \item Combien a-t-il souffert pendant son enfance !
        \item Que d’exploits a-t-il accomplis !
        \item À quels drames du passé n’ont-ils pas échappé !
    \end{itemize}
    \item impossible avec \textit{comme} ou \textit{ce que}, et dans une subordonnée exclamative
    \begin{itemize}
        \item * Comme faut-il se méfier des journalistes !
        \item * Ce qu’est-il intelligent, ce type !
        \item * Je sais [combien a-t-il souffert].
    \end{itemize}
\end{enumerate}

\subsubsection{Le verbe à sujet suffixé en subordonnée circonstancielle}
\paragraph{Le verbe à sujet suffixé dans une subordonnée concessive}

\begin{enumerate}
    \item La subordonnée commence par un adjectif modifié par \textit{aussi} ou \textit{si}, plus rarement \textit{pour}, \textit{quelque}, \textit{tout}, suivi du verbe être au subjonctif 
    \begin{itemize}
        \item Ces récits, [aussi vrais soient-ils], sont trop pittoresques à mon gré. 
        \item Il s’imaginait donc mal dépendant trop longtemps d’un homme [aussi illustre fût-il]
        \item Tout petit soit-il (une vingtaine de places seulement !), ce bistro du quartier Saint-Roch est difficile à manquer.
    \end{itemize}
    \begin{enumerate}
        \item sujet suffixé à construction \textit{aussi} + adjectif 可与les subordonnées où le syntagme adjectival est suivi de \textit{que} 互换
    \begin{itemize}
        \item Ces récits, aussi vrais qu’ils soient, sont trop pittoresques.
    \end{itemize}
    \end{enumerate}
    \item La subordonnée peut aussi être sans introducteur
    \begin{itemize}
        \item Renoncerait-elle au poste, le directeur ne cèderait pas.
        \item Je ne le recevrai pas, [fût-il ministre].
    \end{itemize}
    \item Il est rare, mais pas impossible, de rencontrer un sujet nominal avant le verbe à sujet suffixé 
    \begin{itemize}
        \item Si doué Paul soit-il, il devra travailler pour son examen.
        \item Marie renoncerait-elle, le directeur ne cèderait pas.
    \end{itemize}
\end{enumerate}

\paragraph{Le verbe à sujet suffixé dans une subordonnée conditionnelle ou temporelle}
sujet suffixé dans deux phrases liées conditionnelle ou temporelle. 
\begin{enumerate}
    \item le verbe est à l’imparfait, au plus-que-parfait ou au subjonctif plus-que-parfait.
    \begin{itemize}
        \item Était-il invité dans un salon, Paul fronçait le sourcil de peur. (conditionnelle)
        \item À peine avait-il disparu, le galop d’un cheval a fait trembler le sol mou (temporelle)
        \item  et maintenant elle se louait d’avoir appris et de s’être gardée, puisque [eût-elle agi différemment] le don n’aurait pas été si merveilleux 
    \end{itemize}
    \item 被分析为主句的部分可被que引导
    \begin{itemize}
        \item Était-il invité dans un salon qu’il fronçait le sourcil de peur.
        \item À peine avait-il disparu que le galop d’un cheval a fait trembler le sol mou.
    \end{itemize}
    \item Un sujet nominal est possible avant le verbe à sujet suffixé
    \begin{itemize}
        \item À peine Grange était-il de retour que la frontière, de nouveau, fut alertée pour plusieurs jours
        \item Paul prenait-il la parole, tout le monde faisait silence.
    \end{itemize}
\end{enumerate}

\subsection{L’inversion du sujet nominal}

\subsubsection{Un sujet nominal inversé}

Le sujet nominal inversé peut aussi être
\begin{enumerate}
    \item un nom propre
    \begin{itemize}
        \item Puis est entrée [Marie].
    \end{itemize}
    \item un pronom fort
    \begin{itemize}
        \item Ensuite n’est entré que [toi].
        \item À quoi sert [cela] ?
    \end{itemize}
    \item une relative sans antécédent
    \begin{itemize}
        \item Viendra [qui veut] !
    \end{itemize}
\end{enumerate}

\paragraph{L’accord du verbe avec le sujet nominal inversé}
\begin{enumerate}
    \item quand nominal, le verbe est toujours à la 3e personne
    \begin{itemize}
        \item Quand arrivera le nouvel élève ?
        \item Quand arriveront les nouveaux élèves ?
    \end{itemize}
    \item quand pronom fort de 1re ou 2e personne, le verbe reste à la 3e personne
    \begin{itemize}
        \item Ce soir, moi seul viendrai.
        \item Ce soir ne viendra que moi/*ne viendrai que moi.
    \end{itemize}
    \item le réfléchi est toujours \textit{se}
    \begin{itemize}
        \item C’est un endroit où seul toi te baignes.
        \item C’est un endroit où ne se baigne que toi.
        \item * C’est un endroit où ne te baignes que toi.
    \end{itemize}
\end{enumerate}

\paragraph{La position du sujet nominal inversé}

\begin{enumerate}
    \item Aux temps composés, le sujet nominal inversé suit l’auxiliaire et le participe passé, et au passif, le participe passif
    \begin{itemize}
        \item Quand est partie Marie ?
        \item * Quand est Marie partie ?
        \item Seuls seront reçus les députés suivants.
        \item * Seuls seront les députés suivants reçus.
    \end{itemize}
    \item Il peut suivre ou précéder un complément du verbe, sauf dans l’inversion élaborative, qui l’oblige à suivre le complément
    \begin{itemize}
        \item Quand parlera [Marie] [aux enfants] ?
        \item Quand parlera [aux enfants] [Marie] ?
        \item Recevront [leur diplôme] les étudiants suivants
        \item * Recevront les étudiants suivants [leur diplôme]
    \end{itemize}
\end{enumerate}

\paragraph{Le verbe à sujet inversé n’admet pas tous les compléments.}
\begin{enumerate}
    \item un complément nominal n’est généralement pas possible
    \begin{itemize}
        \item * Où interroge [les élèves] [le professeur] ?
        \item * Sur la place organise [un concert] [la mairie].
    \end{itemize}
    \item hormis dans l’inversion élaborative
    \begin{itemize}
        \item Ont reçu [un prix] [les élèves de 6e 5 et de 5e 4].
    \end{itemize}
    \item un complément nominal sans déterminant, ou figé, ou une proforme faible interprétée comme complément, est parfois acceptable.
    \begin{itemize}
        \item Où font [cours] [les professeurs] ?
        \item Sur la place a [lieu] [un concert].
        \item Où vous interroge [le professeur] ?
    \end{itemize}
    
\end{enumerate}

\paragraph{Le sujet préverbal et le sujet inversé}

\begin{enumerate}
    \item Comme le sujet préverbal, le sujet nominal inversé ne peut pas être pronominalisé par \textit{en},  sauf dans l’inversion dite présentative
    \begin{itemize}
        \item Qu’ont fait [beaucoup de Français] pendant les vacances ?
        \item * Qu’en ont fait beaucoup ?
        \item Alors entrèrent [deux autres soldats].
        \item Alors en entrèrent deux autres.
    \end{itemize}
    \item Contrairement au sujet préverbal, le sujet inversé peut être introduit par \textit{de}, mais non par \textit{seul} 
    \begin{itemize}
        \item * Jamais [d’étudiants] ne dorment ici.
        \item Où ne dorment jamais [d’étudiants] ?
        \item Seuls tes amis sont venus ici ce soir.
        \item * Où sont venus [seuls tes amis] ?
    \end{itemize}
    \item Contrairement au sujet préverbal, au pluriel, il ne permet pas le quantifieur tous en usage disjoint
    \begin{itemize}
        \item * Quel examen ont tous réussi [les étudiants] ?
        \item Les étudiants ont tous réussi leur examen.
    \end{itemize}
    \item Le sujet inversé ne peut pas non plus être réduit à un déterminant indéfini (\textit{plusieurs}), à un cardinal ou au pronom \textit{tous}
    \begin{itemize}
        \item Parmi ces tableaux, [plusieurs/trois] ont été achetés hier.
        \item * Hier ont été achetés [plusieurs/trois].
        \item Tous viendront demain.
        \item * Quand viendront tous ?
    \end{itemize}
\end{enumerate}

\paragraph{Les constructions à sujet nominal inversé}
...
\begin{table}[H]
    ...
    \hline
    \rowcolor{cyan!20}
    \textbf{INVERSION} & \textbf{PHRASE RACINE} & \textbf{SUBORDONNÉE} \\
    \hline
    attributive & Rares sont les enfants orphelins. & On a relevé [que rare est la panne brutale]. \\
    \hline
    élaborative & Sont prioritaires les enfants en bas âge. & Il est clair [que sont prioritaires les enfants en bas âge]. \\
    \hline
    en incise & --- & Nous vaincrons, [disait Paul]. \\
    \hline
    locative & Ici se dresse un obstacle. & Il est clair [qu'ici se dresse un obstacle]. \\
    & Au désespoir succéda l'enthousiasme. & Je sais [qu'à 20 h aura lieu un concert]. \\
    \hline
    de manière & Ainsi se conduisent les hommes. & On dit [qu'ainsi en a décidé le Président]. \\
    \hline
    présentative & Entra alors un écuyer. & Il me semble [qu'alors entra un écuyer]. \\
    & Que vienne Marie ! & J'attends [que vienne la nuit]. \\
    \hline
    stylistique & Où va Paul ? & On se demande [où va Paul]. \\
    & Quelle allure a Paul ! & Voici l'homme [qu'est devenu Paul]. \\
    & & Il parlait [comme parlent les livres]. \\
    \hline
    \end{tabular}
    \caption{Les principales constructions à inversion du sujet nominal}
\end{table}

\begin{enumerate}
    \item Les inversions élaborative et attributive 只用于les phrases déclaratives et exclamatives，不能用于interrogatives ou désidératives
    \item Les autres inversions sont possibles dans tous les types de phrase
\end{enumerate}

\subsubsection{L’inversion élaborative}
inversion énumérative, inversion de liste ou encore permutation de longueur. Elle est compatible avec un complément nominal direct ou un attribut nominal

\begin{enumerate}
    \item le sujet nominal en fin de phrase après les compléments du verbe, 不能置于complément前
    \begin{itemize}
        \item Vont recevoir une grosse prime à la fin de l’année [Jean et tous ses collègues].
        \item * Vont recevoir [Jean et tous ses collègues] [une grosse prime à la fin de l’année].
        \item Est élevé au rang et à l’appellation de général de corps aérien [le général de division aérienne J.-F. L. ]
        \item Il est écrit que sont reçus [les candidats suivants].
        \item Par décret sont nommés officiers [les sous-officiers suivants].
        \item * Sont nommés [les sous-officiers suivants] [officiers].
    \end{itemize}
    \item 可与 \textit{seul, entre autres, notamment, aussi, également}连用 
    \begin{itemize}
        \item Seront seuls reçus par le ministre [les journalistes accrédités].
        \item Sont entre autres acceptés [les chèques bancaires et les chèques restaurant].
        \item La carte de crédit est acceptée. Sont également acceptés [les chèques bancaires et les chèques restaurant].
        \item Est seule/seulement acceptée [la carte de crédit].
    \end{itemize}
    \item 很难与negation连用，Il faut en effet qu’au moins un élément soit concerné par le prédicat verbal.
    \begin{itemize}
        \item \# Dans cette entreprise, n’ont pas été augmentés depuis dix ans [les caissières et les chauffeurs].
        \item \# Sans électricité, ne pouvait fonctionner normalement [aucun dispositif de sécurité].
    \end{itemize}
    \begin{enumerate}
        \item la négation est possible si d’autres éléments sont concernés.
        \begin{itemize}
            \item Par décision du 8 mai, ne sont plus commercialisables [l’acajou ou l’okoumé].
            \item Sans électricité, ne pouvaient fonctionner normalement [ni les ascenseurs ni les caméras de surveillance].
        \end{itemize}
        \item 也可使用\textit{n’est pas... qui veut}. avec la relative sans antécédent \textit{qui veut} comme sujet inversé.
        \begin{itemize}
            \item N’est pas Shakespeare [qui veut].
        \end{itemize}
    \end{enumerate}
\end{enumerate}

\subsubsection{L’inversion stylistique}
Quand le constituant initial est extrait, le sujet peut généralement être inversé
\begin{enumerate}
    \item À la différence de l’inversion élaborative, elle est compatible avec la négation de phrase
    \begin{itemize}
        \item C’est là [où les enfants ne doivent pas aller]. |C’est là [où ne doivent pas aller les enfants].
    \end{itemize}
    \item Le sujet inversé peut permuter avec un complément prépositionnel, mais pas un complément nominal référentiel
    \begin{itemize}
        \item Quand parlera Marie [aux étudiants] ?
        \item Quand parlera [aux étudiants] Marie ?
        \item * Quand lira [ces livres] Marie ?
    \end{itemize}
    \item Le sujet inversé est plus difficilement avec un attribut nominal, sauf s’il est sans déterminant
    \begin{itemize}
        \item * C’est le jour [où est devenu mon ami Paul].
        \item Je connais le village [dont était maire Paul Dupont].
        \item Je connais le garçon [dont est amoureuse Marie].
        \item À quelle date sont devenus populaires les chanteurs de tango ?
    \end{itemize}
\end{enumerate}

\paragraph{L’inversion stylistique et le extrait}
\begin{enumerate}
    \item extrait可以是un complément de verbe, de nom ou d’adjectif
    \begin{itemize}
        \item C’est le village [dont est maire ◊ Paul].
        \item C’est la fille [dont est amoureux ◊ Jean].
    \end{itemize}
    \item extrait可以是ajout de manière, de degré (mesure), de localisation spatiale ou temporelle
    \begin{itemize}
        \item La manière [dont parle ◊ Paul] est révélatrice de ses origines.
        \item On ne peut pas mesurer la vitesse [à laquelle se déplacent ◊ les électrons].
        \item On ne peut pas mesurer la vitesse [à laquelle se déplacent ◊ les électrons].
        \item Il a repeint la pièce [dans laquelle dorment ◊ les enfants].
        \item C’était le dimanche [où sont partis ◊ les enfants].
    \end{itemize}
    \item extrait不能是ajout de cause, de point de vue ou de perspective. 
    \begin{itemize}
        \item * Je ne connais pas les raisons [pour lesquelles s’est mis en colère le président].
        \item * Paul a demandé l’autorisation [sans laquelle ne peuvent s’absenter les enfants mineurs].
        \item * L’hypothèse [selon laquelle tourne la terre] ne s’est pas imposée facilement.
    \end{itemize}
    \begin{enumerate}
        \item 因此 \textit{pourquoi, comment}不允许l’inversion stylistique, 除非comment表示manière
        \begin{itemize}
            \item * Je me demande [pourquoi a parlé Marie].
            \item * Cette réunion, je me demande encore [comment a pu l’oublier Paul].
            \item Je sais [comment marche ◊ cette machine].
        \end{itemize}
    \end{enumerate}
\end{enumerate}


\paragraph{L’inversion stylistique dans les interrogatives et les exclamatives}
\begin{enumerate}
    \item 可用于les interrogatives partielles mais non dans les interrogatives totales. interrogatif必须出现在句首
    \begin{itemize}
        \item À qui parlera Marie ?
        \item * Est-ce que parlera Marie ?
        \item * Parlera Marie quand ?
        \item * Parlera quand Marie ?
    \end{itemize}
    \begin{enumerate}
        \item 可与 \textit{est-ce que, \textup{tel} qu’est-ce que \textup{ou} où est-ce que}连用
        \begin{itemize}
            \item Qu’est-ce que fera Marie ? |Qu’est-ce que Marie fera ?
            \item Où est-ce que va Marie ? |Où est-ce que Marie va ?
        \end{itemize}
        \item interrogatifs faibles \textit{que} et \textit{quel}必须用l’inversion stylistique
        \begin{itemize}
            \item Que fait Marie ? |*Que Marie fait ?
            \item Quelle est la température ? |*Quelle la température est ?
        \end{itemize}
    \end{enumerate}
    \item 多用于les exclamatives en \textit{combien}, \textit{que} ou \textit{quel}. Avec \textit{comme}, \textit{ce que} et \textit{qu’est-ce que}, l’inversion est plus rare et limitée aux verbes gradables
    \begin{itemize}
        \item Combien sont arbitraires en ces choses les conventions de la pudeur !
        \item Que de problèmes fait éclater chacun de ces mots !
        \item Que me pèsent toutes ces réunions !
        \item Quel souvenir amer lui laissait cette aventure !
        \item Ce que me coutent ces réunions incessantes !
        \item Comme me pèsent ces réunions incessantes !
    \end{itemize}
    \item 只用于mot interrogatif ou exclamatif引导的les subordonnées interrogatives ou exclamatives，不能用于 \textit{si} ou \textit{que}引导的从句 
    \begin{itemize}
        \item Je me demande [à qui parlera Marie].
        \item Je comprends [combien a souffert Marie].
        \item * Je me demande [si parlera Marie].
        \item * Je comprends [qu’a tant souffert Marie].
    \end{itemize}
\end{enumerate}


\paragraph{L’inversion stylistique dans les subordonnées relatives et les clivées}
\begin{enumerate}
    \item Les subordonnées relatives除了 \textit{dont} avec proforme引导的，其余都可用L’inversion stylistique
    \begin{itemize}
        \item La pièce [où dorment les enfants] doit être repeinte.
        \item Le roman [qu’a écrit Paul] sera bientôt publié.
    \end{itemize}
    \item Les constructions clivées qui incluent une subordonnée relative也可使用
    \begin{itemize}
        \item C’est là [où nous attend Marie].
        \item C’est le danger [que court tout écrivain].
    \end{itemize}
\end{enumerate}

\subsubsection{L’inversion stylistique dans les subordonnées circonstancielles et les comparatives}

\begin{enumerate}
    \item concessives au subjonctif avec syntagme adjectival initial suivi de \textit{que} 
    \begin{itemize}
        \item Pour isolée et presque sauvage que fût leur existence, la grand-mère et sa petite-fille ne pouvaient éviter un certain nombre de contacts et même de relations.
    \end{itemize}
    \item causales à l’indicatif avec adverbe initialf
    \begin{itemize}
        \item Il ne répondit pas [tant sont complexes les ramifications de l’affaire].
    \end{itemize}
    \item comparatives en \textit{comme} ou \textit{que}
    \begin{itemize}
        \item On ne pouvait plus avancer, malade [comme était Paul].
        \item Il est aussi rusé [qu’était son père].
    \end{itemize}
\end{enumerate}

\subsubsection{L’inversion attributive}
certains adjectifs peuvent apparaitre en début de phrase, et déclencher l’inversion du sujet
\begin{enumerate}
    \item L’attribut de sujet est généralement un syntagme adjectival, plus rarement un adverbe ou un syntagme prépositionnel
    \begin{itemize}
        \item Oui, je sais que je ressasse et remâche et me répète. Ainsi est la ruminante douleur
        \item Mon père était fâché. Plus en colère encore était ma mère.
    \end{itemize}
    \begin{enumerate}
        \item L’adjectif initial peut être modifié par un adverbe de degré (\textit{aussi, plus, si}) 
        \begin{itemize}
            \item Et si l’on voyait la grammaire de là-haut ? Si grandes étaient notre obsession, et notre confiance, à l’époque, que nous en aurions juré 
        \end{itemize}
    \end{enumerate}
    \item Un verbe à attribut du complément est parfois possible, à condition que ce dernier soit pronominalisé
    \begin{itemize}
        \item Divorcer car son époux ou son épouse est infidèle est encore possible, tel l’a jugé la Cour d’appel de Paris.
        \item * Tel a jugé le divorce la Cour d’appel de Paris.
    \end{itemize}
    \item L’inversion attributive peut aussi apparaitre en subordonnée, aussi bien complétive que causale, entre autres.
    \begin{itemize}
        \item Très justement, on a relevé [que rare est la panne brutale] 
        \item ils ne purent s’empêcher de penser [que vaine était leur expédition]
        \item J’ai décidé de me tuer [parce que telle est mon idée].
    \end{itemize}
    \item La négation de phrase est généralement difficile avec l’inversion attributive, sauf avec tel
    \begin{itemize}
        \item * Rares ne sont pas les gens de bonne humeur.
        \item Limiter le pouvoir du parlement, telle n’est pas son intention.
    \end{itemize}
\end{enumerate}

\subsubsection{L’inversion locative ou de manière}

\subsubsection{L’inversion locative}
un syntagme prépositionnel en début de phrase
\begin{itemize}
    \item Sur la place avait lieu un tournoi.
    \item De ces mouvements tirent leur origine [tous les autres au moyen d’un mécanisme central] 
    \item À un printemps pluvieux succéda un été radieux.
    \item Après le doucissage vient le savonnage dans lequel on substitue au sable des boues provenant de sable ayant déjà servi
    \item De ces mouvements tirent leur origine [tous les autres au moyen d’un mécanisme central] 
    \item Dans la salle à manger nous attend [un spectacle bien plus pénible]. 
\end{itemize}

Le syntagme prépositionnel est extrait et peut être en début de subordonnée ou en début de phrase
\begin{itemize}
    \item On constatera [qu’aux difficultés financières s’ajoutent les impasses juridiques].
    \item On peut se demander [si aux difficultés financières ne s’ajouteront pas rapidement les impasses juridiques].
    \item Et de cet abîme sonore il faut [que jaillisse un écho]. 
\end{itemize}


\paragraph{L’inversion de manière}
\textit{ainsi, de cette manière, de même}  en debut de phrase
\begin{enumerate}
    \item \textit{ainsi} de manière, équivalent \textit{à de cette manière} ou \textit{de la même manière}
    \begin{itemize}
        \item Il écrivait avec une sorte de distraction concentrée, comme on crayonne sur le bloc du téléphone. Ainsi écrivait Alexandre, se réfugiant dans les pleins et les déliés de cette écriture sage, de ce crayonnement appliqué. 
        \item Ainsi prenait corps dans le pays une nouvelle espérance.
    \end{itemize}
    \item \textit{ainsi} anaphorique, compatible avec la proforme \textit{le} qui reprend l’attribut ou la phrase précédente
    \begin{itemize}
        \item Pierre était appelé le chef. Ainsi l’avaient baptisé ses condisciples pleins d’admiration.
        \item Le commerce s’adaptait à leur abêtissement. Ainsi le voulait leur principe sacré de la libre entreprise.
    \end{itemize}
    \item l’inversion de manière est peu naturelle avec la négation de phrase
\end{enumerate}


\subsubsection{L’inversion présentative}

\begin{table}[H]
    \centering
    \small
    \begin{adjustbox}{max width=\textwidth}
        \begin{tabular}{|p{3cm}|>{\RaggedRight\itshape}p{6.5cm}|>{\RaggedRight}p{4cm}|}
        \hline
        \rowcolor{cyan!20}
        \textbf{} & \textbf{\textup{VERBES}} & \textbf{EXEMPLES} \\
        \hline
        \rule{0pt}{1.5ex}de changement d'état & apparaître, augmenter, baisser, casser, cesser, commencer, couler, craquer, devenir, diminuer, disparaître, durcir, empirer, fleurir, fondre, grandir, grossir, mourir, naître, refroidir, s'améliorer, se détériorer, se dissiper, s'éteindre, s'évanouir, s'évaporer, sécher, tomber & Nait alors un espoir. \newline Je sens [qu'augmente l'angoisse]. \\
        \hline
        \rule{0pt}{1.5ex}de changement de lieu & aller, accourir, arriver, courir, descendre, entrer, jaillir, monter, partir, passer, se promener, sortir, surgir & Arrive alors un soldat. \newline J'attends [que vienne la nuit]. \\
        \hline
        \rule{0pt}{1.5ex}d'émission (de son ou de substance) & briller, fumer, résonner, retentir & Retentit soudain un coup de tonnerre. \newline J'attends [que sonne midi]. \\
        \hline
        \rule{0pt}{1.5ex}d'existence et de localisation & avoir lieu, demeurer, durer, être, exister, régner, rester, s'étendre, s'éterniser, se tenir, survivre & Se tiendra ensuite un banquet. \newline Je crains [que ne s'éternise cette réunion]. \\
        \hline
        \end{tabular}
    \end{adjustbox}
    \caption{Les principaux verbes de l’inversion présentative}
\end{table}
