...
\hline
\textbf{d’ordre ou d’influence :} \newline conseiller (\textit{à}) (\textit{de}), convaincre (\textit{de}), dire (\textit{à}) (\textit{de}), interdire (\textit{à}) (\textit{de}), obliger (\textit{à}), ordonner (\textit{à}) (\textit{de}), permettre (\textit{à}) (\textit{de}) & complément direct ou oblique & \textit{Luc permet à Max [\uline{de partir}].} \newline \textit{Luc oblige Max [\uline{à partir}].} \\
\hline
\textbf{de perception :} \newline écouter, entendre, observer, regarder, sentir, voir & complément oblique & \textit{Luc entend Max [\uline{chanter un air}].} \\
\hline
\end{tabular}
