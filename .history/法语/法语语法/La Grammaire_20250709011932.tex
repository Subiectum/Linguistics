...
You're right to point out that \begin{adjustbox}{max width=\textwidth} might not always result in the ideal fit, especially if the content itself is very wide and compacting it distorts the text or makes it unreadable.

The adjustbox package scales the content, which can make the text unreadably small if the table is significantly wider than the \textwidth.

Here are a few strategies to make your LaTeX table fit the PDF page better, ordered from generally recommended to more drastic measures:

1. Reduce Font Size within the Table:
This is often the first and most effective step. You can apply a smaller font size specifically to the table.

Code snippet

\documentclass{article}
\usepackage{array}
\usepackage{adjustbox} % Still useful if you want to combine with scaling later

\begin{document}

\begin{table}[H]
    \centering
    \small % <--- Apply a smaller font size here (e.g., \small, \footnotesize, \scriptsize, \tiny)
    \begin{adjustbox}{max width=\textwidth} % Keep this for initial scaling attempt
        \begin{tabular}{|l|l|l|}
        \hline
        ...
        & & Paul manque {de place}. \\
        & & Cette pièce sert {de débarras}. \\
        \hline
        en & consister, dégénérer, finir, muer, partir & Ce logement consiste {en deux pièces}. \\
        & & Le jeu dégénère {en pugilat}. \\
        \hline
        par & commencer, débuter, finir, terminer & Le spectacle commence {par un concert}. \\
        \hline
        sur & s'accordent, s'acharner, agir, anticiper, s'appuyer, compter, cracher, crier, discourir, s'entendre, foncer, hurler, interroger, insister, marcher, médire, mentir, parler, passer, pleurer, taper, tirer, régner, se reposer, revenir, sauter, tomber & Paul compte {sur son frère}. \\
        & & Paul insiste {sur sa proposition}. \\
        & & Paul tire {sur un pigeon}. \\
        & & Luc saute {sur l'occasion}. \\
        \hline
        ...
        & & Max se conduit {avec tact}. \\
        & & La garantie dure {jusqu'en 2020}. \\
        \hline
        \end{tabular}
    \end{adjustbox}
\end{table}
...
\end{document}
You can try \footnotesize, \scriptsize, or even \tiny if needed. Remember to put \normalsize after the table if you want the rest of your document to revert to the default font size, or enclose the table within \begingroup and \endgroup.

2. Use p{width} for specific column widths and text wrapping:
Instead of l (left-aligned) columns, use p{width} for columns where content might wrap. This gives you more control over column sizes and allows text to break naturally. You'll need to estimate appropriate widths for each column.

Code snippet

\documentclass{article}
\usepackage{array} % For p{} columns
% \usepackage{adjustbox} % Not strictly necessary if you manually set column widths

\begin{document}

\begin{table}[H]
    \centering
    % You might still use \small or \footnotesize here if needed
    \begin{tabular}{|l|p{6cm}|p{6cm}|} % Adjust column widths as needed
    \hline
    \textbf{PRÉPOSITIONS} & \textbf{VERBES} & \textbf{EXEMPLES} \\
    \hline
    à & aller, convenir, correspondre, jouer, manquer, mentir, nuire, obéir, parler, penser, renoncer, ressembler, servir, sourire, suffire, tenir & Cette robe va {à Marie}. \\
