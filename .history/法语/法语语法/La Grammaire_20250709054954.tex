...
\hline
\textbf{d’ordre ou d’influence :} & conseiller (\textit{à}) (\textit{de}), convaincre (\textit{de}), dire (\textit{à}) (\textit{de}), interdire (\textit{à}) (\textit{de}), obliger (\textit{à}), ordonner (\textit{à}) (\textit{de}), permettre (\textit{à}) (\textit{de}) & complément direct ou oblique & \textit{Luc permet à Max de partir.} \newline \textit{Luc oblige Max à partir.} \\
\hline
\textbf{de perception :} & écouter, entendre, observer, regarder, sentir, voir & complément oblique & \textit{Luc entend Max chanter un air.} \\
\hline
\end{tabular}
