...
\begin{enumerate}
    \item complément nominal
    \item complément prépositionnel
    \item complément infinitif avec de ou à
    \item complément subordonnée à l’indicatif ou au subjonctif 
...
Understood. You prefer the \hline format for horizontal lines, which is standard LaTeX table formatting without the booktabs package.

Here's the LaTeX code for the combined table using \hline:
...
\begin{table}[h!]
    \centering
    \caption{Relations Sémantiques et Prépositions Associées}
    \label{tab:sem_relations_hline}
    \begin{tabular}{|l|l|p{6cm}|}
        \hline
        \textbf{RELATION SÉMANTIQUE} & \textbf{PRÉPOSITIONS} & \textbf{EXEMPLES} \\
        \hline
        comparaison, opposition & contre, d'avec, malgré, nonobstant, en dépit (de), au contraire (de), à la différence (de), à côté (de), au fur et à mesure (de), au prorata (de), par rapport (à) & un faible gain \emph{[par rapport aux frais]} \newline partir \emph{[malgré la chaleur]} \\
        \hline
        instrument, moyen & avec, à l'aide (de), au moyen (de), de, par & couper le pain \emph{[avec un couteau]} \\
        \hline
        manière & à la façon (de), à la manière (de), comme, façon, selon & travailler \emph{[à la manière d'autrefois]} \newline travailler \emph{[comme peintre]} \\
        \hline
        possession, transfert de possession & à, chez, en possession (de), pour & un livre \emph{[à moi]} \newline un cadeau \emph{[pour Marie]} \newline trouver du courage \emph{[chez Luc]} \\
        \hline
        prix, quantité, valeur & à, de, fois, moins, moyennant, par, plus, pour, au prix (de), à raison (de), à hauteur (de), sur & une bague \emph{[à 100 euros]} \newline un modèle \emph{[pour 20 euros]} \newline un sac \emph{[de 20 kilos]} \newline trois \emph{[fois trois]} \\
        \hline
        source du comportement ou de l'information & d'après, à l'instar (de), comme, en, en tant que, en qualité (de), selon & agir \emph{[selon ses principes]} \newline agir \emph{[d'après la loi]} \\
        \hline
        substitution & pour, au lieu (de), à la place (de), au nom (de), en replacement (de) & une école \emph{[au lieu d'un parking]} \newline un mort \emph{[pour un vivant]} \newline parler \emph{[au nom des enfants]} \\
        \hline
        topique & concernant, de, à propos (de), au sujet (de), à l'égard (de), envers, quant (à), sur & une décision \emph{[à propos des plantes]} \newline un livre \emph{[de maths]} \newline un traité \emph{[sur les plantes]} \\
        \hline
        absence, exclusion & à part, à l'exception (de), excepté, exception faite (de), hormis, sans, sauf & un voyage \emph{[sans les enfants]} \\
        \hline
        accompagnement, inclusion & avec, d'entre, parmi, y compris & une promenade \emph{[avec le chien]} \\
        \hline
        avantage & au profit (de), au bénéfice (de), en faveur (de), pour. & un modèle \emph{[pour chiens]} \newline des mesures \emph{[en faveur des réfugiés]} \\
        \hline
        désavantage & aux dépens (de), au détriment (de), contre & un vote \emph{[contre le projet]} \\
        \hline
        but, direction, motivation, orientation & à, afin (de), après, envers, pour, sur, vers & courir \emph{[après les honneurs]} \newline de la haine \emph{[envers quelqu'un]} \newline travailler \emph{[pour le plaisir]} \\
        \hline
        cause & de, devant, de par, grâce à, à / pour cause (de), à force (de), au gré (de), en cas (de), en raison (de), en vertu (de), par, pour, suite (à) & agir \emph{[au gré de son humeur]} \newline rouge \emph{[de colère]} \newline une branche cassée \emph{[par le vent]} \\
        \hline
    \end{tabular}
\end{table}
