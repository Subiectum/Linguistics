...
% Define a new column type for centered text
...
\begin{tabular}{| L{3.8cm} | L{3cm} | L{2.5cm} | L{5.5cm} |}
\hline
\multicolumn{2}{|c|}{\textbf{VERBES}} & \textbf{INFINITIF} & \textbf{EXEMPLES} \\
\hline
\multicolumn{4}{|c|}{\textbf{À UN COMPLÉMENT}} \\
\hline
\textbf{aspectuel :} & aller, commencer (\textit{à / par}), continuer (\textit{à / de}), être en train (\textit{de}), finir (\textit{de / par}), se mettre (\textit{à}), venir (\textit{de}) & complément direct ou oblique & Luc finit [de travailler]. Luc commence [à comprendre]. Cela finira [par se savoir]. \\
\hline
\textbf{de décision :} & accepter (\textit{de}), décider (\textit{de}), opter (\textit{pour}), refuser (\textit{de}), renoncer (\textit{à}), voter (\textit{pour}) & complément direct ou oblique & Luc choisit [de partir]. Luc opte [pour partir]. \\
\hline
\textbf{de déplacement :} & aller, courir, descendre, monter, partir, venir & complément oblique & Luc court [acheter du pain]. \\
\hline
\textbf{de désir et de sentiment :} & aimer, s’attendre (\textit{à}), craindre (\textit{de}), désirer, détester, redouter (\textit{de}), regretter (\textit{de}), souhaiter, tenir (\textit{à}), vouloir & complément direct ou oblique & Luc aime [dormir]. Luc tient [à venir]. \\
\hline
\textbf{d’essai :} & chercher (\textit{à}), essayer (\textit{de}), réussir (\textit{à}), tenter (\textit{de}) & complément direct & Luc essaie [de comprendre]. Luc cherche [à comprendre]. \\
\hline
\textbf{d’identité :} & être, paraitre, passer pour, rester (\textit{à}), sembler & attribut ou complément oblique & Le problème est [de trouver]. Luc semble [ne rien faire]. Luc reste [à ne rien faire]. \\
\hline
\textbf{modal :} & avoir (\textit{à}), devoir, falloir nécessiter (\textit{de}), pouvoir, réclamer (\textit{de}), risquer (\textit{de}) & complément direct & Il faut [partir]. Luc a [à travailler]. Luc risque [d’échouer]. \\
\hline
\textbf{d’opinion et d’activité intellectuelle :} & comprendre, croire, imaginer, oublier, penser, savoir, se souvenir (\textit{de}) & complément direct ou oblique & Luc pense [avoir raison]. Luc se souvient [d’être venu]. \\
\hline
\multicolumn{4}{|c|}{\textbf{À DEUX COMPLÉMENTS}} \\
\hline
\textbf{causatif :} & empêcher (\textit{de}), faire, laisser & complément oblique & Luc laisse Max [partir]. Luc empêche Max [de travailler]. \\
\hline
\textbf{causatif de déplacement :} & envoyer, expédier & complément oblique & Luc envoie Max [acheter du pain]. \\
\hline
\textbf{de communication :} & affirmer (\textit{à}), avouer (\textit{à}), déclarer (\textit{à}), dire (\textit{à}), écrire (\textit{à}), expliquer (\textit{à}), prétendre (\textit{à}) & complément direct & Luc nous affirme [tout comprendre]. \\
\hline
\textbf{d’engagement :} & s’engager (\textit{envers}) (\textit{à}), jurer (\textit{à}) (\textit{de}), promettre (\textit{à}) (\textit{de}) & complément direct & Luc jure à Max [de rester]. \\
\hline
\textbf{d’ordre ou d’influence :} & conseiller (\textit{à}) (\textit{de}), convaincre (\textit{de}), dire (\textit{à}) (\textit{de}), interdire (\textit{à}) (\textit{de}), obliger (\textit{à}), ordonner (\textit{à}) (\textit{de}), permettre (\textit{à}) (\textit{de}) & complément direct ou oblique & Luc permet à Max [\underline{de partir}]. Luc oblige Max [\underline{à partir}]. \\
\hline
\textbf{de perception :} & écouter, entendre, observer, regarder, sentir, voir & complément oblique & Luc entend Max [\underline{chanter un air}]. \\
\hline
\end{tabular}
