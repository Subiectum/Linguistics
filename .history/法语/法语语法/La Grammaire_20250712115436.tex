...
\begin{table}[H]
    \centering
    % Define a 4-column table. Vertical lines are included as per request.
    % Column widths are adjusted to accommodate content across different original table structures.
    % Col 1: For 'VERBES' (3-col) or part of 'VERBES' (2-col/4-col INTERROGATIVE)
    % Col 2: For 'INFINITIF' (3-col) or part of 'VERBES' (2-col) or 'TOTALE' (4-col)
    % Col 3: For 'EXEMPLES' (3-col part) or 'EXEMPLES' (2-col part) or 'PARTIELLE' (4-col)
    % Col 4: For 'EXEMPLES' (3-col part) or 'EXEMPLES' (2-col part) or 'ALTERNATIVE' (4-col)
    \begin{tabular}{| L{3.8cm} | L{2.2cm} | L{4.5cm} | L{4.5cm} |}
    \hline
    \textbf{VERBES} & \textbf{INFINITIF} & \multicolumn{2}{c|}{\textbf{EXEMPLES}} \\ % 'EXEMPLES' header spans two columns for the 3-column original tables
    \hline
    \multicolumn{4}{|c|}{\textbf{--- D'APRÈS TABLES À 3 COLONNES (VERBES | INFINITIF | EXEMPLES) ---}} \\ % Section header
    \hline
    \multicolumn{4}{|c|}{\textbf{À UN COMPLÉMENT}} \\ % Merged title from original table
    \hline
    \textbf{aspectuel :} \newline \textit{aller, commencer (à / par), continuer (à / de), être en train (de), finir (de / par), se mettre (à), venir (de)} & complément direct \newline ou oblique & \multicolumn{2}{L{9cm}|}{\textit{Luc finit [de travailler].} \newline \textit{Luc commence [à comprendre].} \newline \textit{Cela finira [par se savoir].}} \\
    \hline
    \textbf{de décision :} \newline \textit{accepter (de), décider (de), opter (pour), refuser (de), renoncer (à), voter (pour)} & complément direct \newline ou oblique & \multicolumn{2}{L{9cm}|}{\textit{Luc choisit [de partir].} \newline \textit{Luc opte [pour partir].}} \\
    \hline
    \textbf{de déplacement :} \newline \textit{aller, courir, descendre, monter, partir, venir} & complément \newline oblique & \multicolumn{2}{L{9cm}|}{\textit{Luc court [acheter du pain].}} \\
    \hline
    \textbf{de désir et de sentiment :} \newline \textit{aimer, s’attendre (à), craindre (de), désirer, détester, redouter (de), regretter (de), souhaiter (à), vouloir} & complément direct \newline ou oblique & \multicolumn{2}{L{9cm}|}{\textit{Luc aime [dormir].} \newline \textit{Luc tient [à venir].}} \\
    \hline
    \textbf{d’essai :} \newline \textit{chercher (à), essayer (de), réussir (à), tenter (de)} & complément \newline direct & \multicolumn{2}{L{9cm}|}{\textit{Luc essaie [de comprendre].} \newline \textit{Luc cherche [à comprendre].}} \\
    \hline
    \textbf{d’identité :} \newline \textit{être, paraitre, passer pour, rester (à), sembler} & attribut \newline ou complément \newline oblique & \multicolumn{2}{L{9cm}|}{\textit{Le problème est [de trouver].} \newline \textit{Luc semble [ne rien faire].} \newline \textit{Luc reste [à ne rien faire].}} \\
    \hline
    \textbf{modal :} \newline \textit{avoir (à), devoir, falloir nécessiter (de), pouvoir, réclamer (de), risquer (de)} & complément \newline direct & \multicolumn{2}{L{9cm}|}{\textit{Il faut [partir].} \newline \textit{Luc a [à travailler].} \newline \textit{Luc risque [d’échouer].}} \\
    \hline
    \textbf{d’opinion et d’activité intellectuelle :} \newline \textit{comprendre, croire, imaginer, oublier, penser, savoir, se souvenir (de)} & complément direct \newline ou oblique & \multicolumn{2}{L{9cm}|}{\textit{Luc pense [avoir raison].} \newline \textit{Luc se souvient [d’être venu].}} \\
    \hline
    \multicolumn{4}{|c|}{\textbf{À DEUX COMPLÉMENTS}} \\ % Merged title from original table
    \hline
    \textbf{causatif :} \newline \textit{empêcher (de), faire, laisser} & complément \newline oblique & \multicolumn{2}{L{9cm}|}{\textit{Luc laisse Max [partir].} \newline \textit{Luc empêche Max [de travailler].}} \\
    \hline
    \textbf{causatif de déplacement :} \newline \textit{envoyer, expédier} & complément \newline oblique & \multicolumn{2}{L{9cm}|}{\textit{Luc envoie Max [acheter du pain].}} \\
    \hline
    \textbf{de communication :} \newline \textit{affirmer (à), avouer (à), déclarer (à), dire (à), écrire (à), expliquer (à), prétendre (à)} & complément \newline direct & \multicolumn{2}{L{9cm}|}{\textit{Luc nous affirme [tout comprendre].}} \\
    \hline
    \textbf{d’engagement :} \newline \textit{s’engager (envers) (à), jurer (à) (de), promettre (à) (de)} & complément \newline direct & \multicolumn{2}{L{9cm}|}{\textit{Luc jure à Max [de rester].}} \\
    \hline
    \textbf{d’ordre ou d’influence :} \newline \textit{conseiller (à) (de), convaincre (de), dire (à) (de), interdire (à) (de), obliger (à), ordonner (à) (de), permettre (à) (de)} & complément \newline direct ou oblique & \multicolumn{2}{L{9cm}|}{\textit{Luc permet à Max [\uline{de partir}].} \newline \textit{Luc oblige Max [\uline{à partir}].}} \\
    \hline
    \textbf{de perception :} \newline \textit{écouter, entendre, observer, regarder, sentir, voir} & complément \newline oblique & \multicolumn{2}{L{9cm}|}{\textit{Luc entend Max [\uline{chanter un air}].}} \\
    \hline
    \multicolumn{4}{|c|}{\textbf{--- D'APRÈS TABLES À 2 COLONNES (VERBES | EXEMPLES) ---}} \\ % Section header
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{d'accompagnement :} \newline \textit{accompagner, escorter}} & \multicolumn{2}{L{9cm}|}{\textit{Le colis est accompagné [\uline{d'un mode d'emploi}].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{d'assistance :} \newline \textit{aider, appuyer, assister, seconder}} & \multicolumn{2}{L{9cm}|}{\textit{Le directeur est aidé [\uline{d'un assistant}].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{donnant la composition d'un groupe :} \newline \textit{composer, constituer, former}} & \multicolumn{2}{L{9cm}|}{\textit{Le gouvernement est constitué [\uline{de quinze ministres}].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{instrumentaux :} \newline \textit{entourer, recouvrir}} & \multicolumn{2}{L{9cm}|}{\textit{Le jardin est recouvert [\uline{de feuilles}].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{de localisation :} \newline \textit{précéder, suivre}} & \multicolumn{2}{L{9cm}|}{\textit{Le diner sera suivi [\uline{d'un concert}].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{d'opinion ou d'activité intellectuelle :} \newline \textit{admettre, approuver, comprendre, connaître, rejeter, tolérer}} & \multicolumn{2}{L{9cm}|}{\textit{La situation n'a pas été comprise [\uline{de la population}].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{de perception :} \newline \textit{écouter, entendre, voir}} & \multicolumn{2}{L{9cm}|}{\textit{Son cri a été entendu [\uline{de tous}].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{de sentiment :} \newline \textit{aimer, apprécier, craindre, détester, envier, inquiéter, mépriser, surprendre}} & \multicolumn{2}{L{9cm}|}{\textit{Ce livre est apprécié [\uline{de tous}].} \newline \textit{Je suis étonné [\uline{du départ de Max}].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{causatifs :} \newline \textit{empêcher, éviter, faire, tâcher}} & \multicolumn{2}{L{9cm}|}{\textit{Paul évite [que Marie vienne].} \newline \textit{La tempête n'a pas empêché [que ce rendez-vous soit un succès].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{de désir et de volonté :} \newline \textit{attendre, chercher (à ce que), craindre, désirer, prier, souhaiter, tenir (à ce que), veiller (à ce que), viser (à ce que), vouloir}} & \multicolumn{2}{L{9cm}|}{\textit{Paul veut [que Marie vienne].} \newline \textit{Paul tient [à ce que Marie vienne].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{d'identité :} \newline \textit{être, rester, sembler}} & \multicolumn{2}{L{9cm}|}{\textit{La consigne est [qu'on ait fini pour demain].} \newline \textit{L'objectif reste [qu'on soit plus nombreux].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{d'influence et d'ordre :} \newline \textit{attendre (de qqn), autoriser (à ce que), conseiller (à qqn), convaincre, défendre (à qqn), demander (à qqn), dire (à qqn), encourager (à ce que), enjoindre à, exiger (de qqn), exhorter (à), forcer (à ce que), interdire (à qqn), inviter (à ce que), ordonner (à qqn), permettre (à qqn), persuader, prier, proposer (à qqn), réclamer (à qqn), recommander (à qqn), sommer}} & \multicolumn{2}{L{9cm}|}{\textit{Luc nous ordonne [que tout soit prêt].} \newline \textit{Luc nous encourage [à ce que tout soit prêt].} \newline \textit{Luc exige de nous [que tout soit prêt].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{d'activité intellectuelle et d'opinion :} \newline \textit{admettre, s’apercevoir, apprendre, calculer, comprendre, constater, contester, croire, découvrir, douter, estimer, évaluer, exclure, ignorer, imaginer, inventer, juger, oublier, penser, savoir, se rappeler, se souvenir (de), supposer, trouver, vérifier}} & \multicolumn{2}{L{9cm}|}{\textit{Je sais [que Paul est rentré].} \newline \textit{Je crois [qu'il va pleuvoir].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{de communication :} \newline \textit{affirmer, annoncer, avertir, dire, s’écrier, écrire, s’exclamer, expliquer, informer, murmurer, se plaindre (de), prétendre, prévenir, protester, raconter}} & \multicolumn{2}{L{9cm}|}{\textit{Marie dit [qu'elle a faim].} \newline \textit{Paul lui murmura [qu'il devait partir].} \newline \textit{Luc se plaint [de ce qu'on l'a prévenu trop tard].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{de choix et de décision :} \newline \textit{choisir, contrôler, décider, décréter, garantir, renoncer (à)}} & \multicolumn{2}{L{9cm}|}{\textit{J'ai décidé [que tout devait être fini pour demain].} \newline \textit{On renonce [à ce que la réunion ait lieu ce soir].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{d'engagement :} \newline \textit{assurer, s’engager (à), jurer, promettre}} & \multicolumn{2}{L{9cm}|}{\textit{Je vous promets [que je serai à la hauteur].} \newline \textit{Je m'engage [à ce que tout soit fini demain].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{d'identité :} \newline \textit{être, rester, sembler}} & \multicolumn{2}{L{9cm}|}{\textit{Le problème est [qu'on n'a plus le temps].} \newline \textit{L'intérêt de l'infiltration reste [qu'on apaise la douleur].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{modaux :} \newline \textit{il faut, il importe, nécessiter, il se peut}} & \multicolumn{2}{L{9cm}|}{\textit{Il se peut [que Paul vienne à Paris].} \newline \textit{Il faut [que vous preniez une assurance].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{de perception et présentatifs :} \newline \textit{entendre, observer, sentir, voici, voilà, voir}} & \multicolumn{2}{L{9cm}|}{\textit{Luc entend [que Max joue du piano].} \newline \textit{Paul voit [que Marie est partie].}} \\
    \hline
    \multicolumn{2}{|L{6cm}|}{\textbf{de sentiment et de réaction émotive :} \newline \textit{admirer, apprécier, détester, s’étonner (de), regretter, se réjouir (de)}} & \multicolumn{2}{L{9cm}|}{\textit{Paul regrette [que Marie soit partie].}} \\
    \hline
    \multicolumn{4}{|c|}{\textbf{--- D'APRÈS TABLE À 4 COLONNES (INTERROGATIVE) ---}} \\ % Section header
    \hline
    \textbf{INTERROGATIVE} & \textbf{TOTALE} & \textbf{PARTIELLE} & \textbf{ALTERNATIVE} \\
    \hline
    est-ce que & \textit{Est-ce qu'il part ?} & \textit{Où est-ce que tu vas ?} & \textit{Est-ce qu'il part ou non ?} \newline \textit{Est-ce que tu vas à Aix ou à Nice ?} \\
    \hline
    sujet suffixé & \textit{Paul part-il ?} & \textit{Où vas-tu ?} & \textit{Paul part-il ou non ?} \newline \textit{Vas-tu à Aix ou à Nice ?} \\
    \hline
    sujet inversé & — & \textit{Où va Paul ?} & — \\
    \hline
    avec seulement mot \newline interrogatif & — & \textit{Où tu vas ?} \newline \textit{Tu vas où ?} & — \\
    \hline
    \multicolumn{4}{|c|}{\textbf{--- D'APRÈS TABLE À 4 COLONNES (INTERROGATIVE) - Deuxième Partie ---}} \\ % Section header for the new interrogative table
    \hline
...
\begin{enumerate}
    \item Les subordonnées interrogatives partielles中，que和quoi替换为ce que, ce qui；quoi只有在被modifié时才能使用
    \begin{itemize}
