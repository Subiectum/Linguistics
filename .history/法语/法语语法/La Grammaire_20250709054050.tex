...
\begin{table}[H] % [H] to place the table exactly here
    \centering
    \small % Adjust font size for the table if needed
    \renewcommand{\arraystretch}{1.2} % Increase vertical spacing slightly for readability

    \begin{tabularx}{\textwidth}{l Y Y} % Use Y for the two auto-wrapping columns
        \toprule % Top horizontal line
        \textbf{PRÉPOSITIONS} & \textbf{VERBES} & \textbf{EXEMPLES} \\
        \midrule % Middle horizontal line (after header)
        à & aller, convenir, correspondre, jouer, manquer, mentir, nuire, obéir, parler, penser, renoncer, ressembler, servir, sourire, suffire, tenir & Cette robe va \{à Marie\}. \newline Paul ressemble \{à son frère\}. \newline Cette pièce sert \{à mon frère\}.\\
        \addlinespace % Adds a little vertical space between rows for clarity
        après & s'acharner, \%, chercher, courir, \%demander, passer, pleurer, venir & Les parents passent \{après les enfants\}. \newline Léa court \{après la renommée\}.\\
        \addlinespace
        avec & discuter, négocier, parler, permuter, rimer & Paul parle \{avec Marie\}. \newline Paul négocie \{avec Marie\}.\\
        \addlinespace
        contre, pour & échanger, opter, parier, se prononcer, voter & Paul opte \{pour votre proposition\}. \newline Paul vote \{contre votre proposition\}.\\
        \addlinespace
        de & convenir, différer, discuter, douter, jouer, manquer, parler, remplir, rêver, rire, servir, tenir, traiter, se vanter, vider & Paul parle \{de son travail\}. \newline Paul manque \{de place\}. \newline Cette pièce sert \{de débarras\}.\\
        \addlinespace
        en & consister, dégénérer, finir, muer, partir & Ce logement consiste \{en deux pièces\}. \newline Le jeu dégénère \{en pugilat\}.\\
        \addlinespace
        par & commencer, débuter, finir, terminer & Le spectacle commence \{par un concert\}.\\
        \addlinespace
        sur & s'accorder, s'acharner, agir, anticiper, s'appuyer, compter, cracher, crier, discourir, s'entendre, foncer, hurler, interroger, insister, marcher, médire, mentir, parler, passer, pleurer, taper, tirer, régner, se reposer, revenir, sauter, tomber & Paul compte \{sur son frère\}. \newline Paul insiste \{sur sa proposition\}. \newline Paul tire \{sur un pigeon\}. \newline Luc saute \{sur l'occasion\}.\\
        \addlinespace
        autres prépositions & choisir (entre), se comporter, se conduire, durer, hésiter (entre), passer (avant), se plaindre (auprès de), se porter, ressusciter (d'entre) & Luc hésite \{entre ces meubles\}. \newline Max se conduit \{avec tact\}. \newline La garantie dure \{jusqu'en 2020\}.\\
        \bottomrule % Bottom horizontal line
    \end{tabularx}
\end{table}

\clearpage % Ensure the next table starts on a new page

% --- Table 2: From Screenshot 2025-07-09 at 05.37.45.png ---
% Style: No vertical lines, some dashed lines, distinct nested headers
\section*{Table 2: Compléments de Verbes}
...
\begin{table}[H] % [H] to place the table exactly here
    ...
    % Using @{} to remove default padding for lines, and specific column widths
    \begin{tabular}{@{ } l @{\hspace{1em}} l @{\hspace{1em}} l @{\hspace{1em}} l @{\hspace{1em}} l @{\hspace{1em}} @{ }} % No vertical lines
        \toprule % Top horizontal line
        \multicolumn{2}{@{}l@{}}{\textbf{}} & \textbf{VERBES} & \textbf{INFINITIF} & \textbf{EXEMPLES} \\
        \midrule % Horizontal line after header
        \multicolumn{5}{@{}c@{}}{\textbf{À UN COMPLÉMENT}} \\
        \cmidrule(lr){1-5} % A line across all columns after the section title
        \addlinespace % Space after line

        aspectuel : & aller, commencer (à) / par, continuer (à) / de, être en train (de), finir (de) / par, \newline se mettre (à) (de) & complément direct \newline ou oblique & Luc finit \{de travailler\}. \newline Luc commence \{à comprendre\}. \newline Cela finira \{par se savoir\}.\\
        \addlinespace
        \cmidrule(lr){1-5} % Dashed line after entry
        \addlinespace
        de décision : & accepter (de), décider (de), opter (pour), refuser (de), renoncer (à), voter (pour) & complément direct \newline ou oblique & Luc choisit \{de partir\}. \newline Luc opte \{pour partir\}.\\
        \addlinespace
        de déplacement : & aller, courir, descendre, monter, partir, venir & complément \newline oblique & Luc court \{acheter du pain\}.\\
        \addlinespace
        de désir et de sentiment : & aimer, s'attendre (à), craindre (de), désirer, détester, redouter (de), regretter (de), \newline souhaiter, tenir (à), vouloir & complément direct \newline ou oblique & Luc aime \{dormir\}. \newline Luc tient \{à venir\}.\\
        \addlinespace
        d'essai : & chercher (à), essayer (de), réussir (à), tenter (de) & complément \newline direct & Luc essaie \{de comprendre\}. \newline Luc cherche \{à comprendre\}.\\
        \addlinespace
        d'identité : & être, paraître, passer pour, rester (à), sembler & attribut \newline ou complément \newline oblique & Le problème est \{de trouver\}. \newline Luc semble \{ne rien faire\}. \newline Luc reste \{à ne rien faire\}.\\
        \addlinespace
        modal : & avoir (à), devoir, falloir nécessiter (de), pouvoir, réclamer (de), risquer (de) & complément direct \newline ou oblique & Il faut \{partir\}. \newline Luc a \{à travailler\}. \newline Luc risque \{d'échouer\}.\\
        \addlinespace
        d'opinion et d'activité intellectuelle : & comprendre, croire, imaginer, oublier, penser, savoir, se souvenir (de) & complément direct \newline ou oblique & Luc pense \{avoir raison\}. \newline Luc se souvient \{d'être venu\}.\\
        \bottomrule % Bottom horizontal line for this section

        \addlinespace % Extra space before the next section
        \addlinespace
        \multicolumn{5}{@{}c@{}}{\textbf{À DEUX COMPLÉMENTS}} \\
        \cmidrule(lr){1-5} % Line after section title
        \addlinespace

        causatif : & empêcher (de), faire, laisser & complément \newline oblique & Luc laisse Max \{partir\}. \newline Luc empêche Max \{de travailler\}.\\
        \addlinespace
        causatif de déplacement : & envoyer, expédier & complément \newline oblique & Luc envoie Max \{acheter du pain\}.\\
        \addlinespace
        de communication : & affirmer (à), avouer (à), déclarer (à), dire (à), écrire (à), expliquer (à), prétendre (à) & complément direct & Luc nous affirme \{tout comprendre\}.\\
        \addlinespace
        d'engagement : & s'engager (envers) (à), jurer (à) (de), promettre (à) (de) & complément direct & Luc jure à \{Max de rester\}.\\
        \bottomrule
    \end{tabular} % Changed to basic tabular as tabularx caused issues with @{} and fixed widths
\end{table}

\clearpage % Ensure the next table starts on a new page

% --- Table 3: From Screenshot 2025-07-09 at 03.04.29.jpg / 03.02.21.jpg ---
% Style: Vertical lines, sections, clinel
\section*{Table 3: Deux Compléments}
...
\begin{table}[H] % [H] to place the table exactly here
    ...
    \begin{tabularx}{\textwidth}{|l|Y|Y|} % Use Y for content columns, with vertical lines
    \hline
    \multicolumn{1}{|c|}{\textbf{}} & \multicolumn{1}{c|}{\textbf{VERBES}} & \multicolumn{1}{c|}{\textbf{EXEMPLES}} \\
    \hline
    \multicolumn{3}{|c|}{\textbf{DEUX COMPLÉMENTS NOMINAUX}} \\
    ...
    \multicolumn{3}{|c|}{\textbf{UN COMPLÉMENT NOMINAL ET UN COMPLÉMENT PRÉPOSITIONNEL}} \\
    \hline
    à & apprendre, avouer, comparer, dire, donner, écrire, emprunter, \newline envoyer, imputer, montrer, offrir, opposer, préférer, prendre, \newline prêter, proposer, rapprocher, voler & Paul préfère \{ceci\} \{à cela\}. \newline Paul donne \{un livre\} \{à Marie\}. \newline Paul écrit \{une lettre\} \{à Marie\}.\\
    \cline{1-3} % Full line across all columns
    avec & associer, comparer, échanger, marier, négocier, permuter, unir & Paul compare \{ceci\} \{avec cela\}. \newline Léa négocie \{un accord\} \{avec Max\}.\\
    \cline{1-3}
    contre, pour & donner, échanger, parler, taper & Paul échange \{ceci\} \{contre cela\}. \newline Paul donne \{30 euros\} \{pour le projet\}. \newline Paul tape \{la règle\} \{contre la table\}.\\
    \cline{1-3}
    de & apprendre, approcher, combler, convaincre, couvrir, différer, \newline informer, obtenir, orner, ôter, prévenir, prolonger, remplir, \newline séparer, sortir, tenir, tirer, traduire, traiter, vider & Paul approche \{l'échelle\} \{du mur\}. \newline Luc remplit \{le salon\} \{de meubles\}. \newline Léa comble \{Max\} \{de cadeaux\}.\\
    \cline{1-3}
    en & aménager, changer, couper, découper, déguiser, diviser, partager, \newline répartir, traduire, transformer & Luc transforme \{la chambre\} \{en bureau\}. \newline Luc partage \{le gâteau\} \{en six parts\}. \newline Paul traduit \{le texte\} \{en anglais\}.\\
    \cline{1-3}
    autres prépositions & choisir (parmi), compter (parmi), descendre, diluer (dans), \newline dissoudre (dans), héberger (inclure (dans)), interroger (sur), loger, \newline mettre, monter, noyer, placer, poser, pousser, prendre (pour), \newline traiter & Paul interroge \{Max\} \{sur sa leçon\}. \newline Paul met \{la vaisselle\} \{dans le placard\}. \newline Luc compte \{Jean\} \{parmi ses amis\}. \newline Paul prend \{Max\} \{pour témoin\}. \newline Paul traite \{Marie\} \{sans égards\}.\\
    \hline
    \multicolumn{3}{|c|}{\textbf{DEUX COMPLÉMENTS PRÉPOSITIONNELS}} \\
    \hline
    à et de & conclure, parler, servir & Cette clef sert \{à Luc\} \{de tournevis\}. \newline Luc parle \{à Max\} \{de son travail\}.\\
    ...
    de et de & hériter & Léa hérite \{d'un buffet\} \{de sa mère\}.\\
    \hline
    de et une autre préposition & arguer (de) (auprès de), convenir (de) (avec), débattre (de) (avec), \newline décider (de) (avec), discuter (de) (avec), hériter (de) (de), \newline se plaindre (de) (auprès de), profiter (de) (pour), \newline répondre (de) (devant), témoigner (de) (auprès de), traiter (de) (avec) & Léa discute \{de tout\} \{avec Max\}. \newline Paul répond \{de ses actes\} \{devant nous\}. \newline Paul témoigne \{de sa bonne foi\} \{auprès du juge\}.\\
    \hline
    autres prépositions & compter (sur) (pour), s'entendre (avec) (sur), parler (avec) (sur) & Luc parle \{avec Max\} \{sur le bleu\}.\\
    \hline
    \end{tabularx}
